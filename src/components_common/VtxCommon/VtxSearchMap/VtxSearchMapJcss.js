...
class VtxSearchMap extends React.Component {
    constructor(props){
        ...
        this.state={
            ...
            isShowList: false,
            //可拖动用于定位的点
            locationPoint: props.locationPoint?props.locationPoint:[],
            locationLine: props.graphicValue?[{
                id: 'locationLine',
                paths:props.graphicValue.geometry.paths[0],
                config:{
                  lineWidth:'2',color:[255,51,255]
                }
            }]:[],
            //locationLine:props.locationLine?props.locationLine:[],
            //搜索框文字
            searchValue: '',
            //搜索出来的列表点位
            listPoint: [],
            ...
            isDraw: props.isDraw&&props.graphicType!='point',
            graphicValue: props.graphicValue,
            /*地图Api参数*/
    ...
    drawLocationLine(){
        this.setState({
            locationLine: [{
                id: 'locationLine',
                paths:this.state.graphicValue?this.state.graphicValue.geometry.paths[0]:[],
                config:{
                  lineWidth:'2',color:[255,51,255]
                }
            }]
    ...
    editGraphicChange(obj){
        ...
        if(obj.geometry.type=='point'){
            ...
            let {locationLine} = this.state;
            locationLine = locationLine.map((item,index)=>{
                return {
                    ...item,
    ...
    render() {
        let {
            ...
            isDoEdit,editGraphicId,graphicValue,isDraw,
            /*modal参数*/
            modal1Visible,
        } = this.state;
        let mapPoints = [...locationPoint,...listPoint];
        let mapLines = [...locationLine];
            ...
            >
                <div className={styles.searchMap}>
                    ...
                    <div className={styles.top}>
                        ...
                        {
                            this.state.graphicType!='point'?<Button onClick={()=>{//debugger
                                this.setState({
                                    isDraw:true,
                                    graphicValue:null,
                                    locationLine:[],
    ...
    componentDidMount(){
        ...
        if (this.props.modal1Visible) {
           ...
           this.state.graphicValue?this.drawLocationLine():null;
        }
    }
    componentDidUpdate(prevProps, prevState) {//重新渲染结束
        if (this.props.modal1Visible && !this.state.locationPoint[0]&& !this.state.locationLine[0]) {
           this.drawLocationPoint();
           this.state.graphicValue?this.drawLocationLine():null;
        }
    }
            ...
            if(nextProps.modal1Visible){
                ...
                this.setState({
                   setZoomLevel: true,
                    ...
                    this.setState({
                        setZoomLevel: false,
                        locationLine: nextProps.graphicValue?[{
                            id: 'locationLine',
                            paths:nextProps.graphicValue.geometry.paths[0],
                            config:{
                              lineWidth:'2',color:[255,51,255]
                            }
                        }]:[]
