...
// 默认上传文件接口
const defaultUploadURL = config.FileURLprefix + '/cloudFile/common/uploadFile';
// 默认下载文件地址，与ID挂钩
const defaultDownloadURL = config.FileURLprefix + '/cloudFile/common/downloadFile?id=';
// 默认nginx代理地址，兼容IE10以下的跨域情况
const proxyUploadURL = `http://${document.location.host}/uploadFilesProxy/`;

class UploadUtil extends React.Component {
    constructor(props) {
        super(props);
        let t = this;
        // 初始化上传下载的地址
    ...
    getConfig() {
        ...
        if (this.props.listType == 'picture-card') {
            config.onPreview = this.handlePreview
        }
    ...
    handlePreview = (file) => {
        this.setState({
            previewImage: defaultDownloadURL + file.id,
            previewVisible: true,
        });
    }
    ...
    render() {
        ...
        return (
            <div className={styles.uploadCt}>
                ...
                {
                    this.props.listType == 'picture-card' && (<Modal visible={previewVisible} footer={null} onCancel={this.handleCancelModal}>
